...
RUN { \
    set -e && \
    cd qt6/src && \
    wget https://download.qt.io/official_releases/qt/6.9/6.9.1/submodules/qtserialport-everywhere-src-6.9.1.tar.xz && \
    wget https://download.qt.io/official_releases/qt/6.9/6.9.1/submodules/qtserialbus-everywhere-src-6.9.1.tar.xz && \
    wget https://download.qt.io/official_releases/qt/6.9/6.9.1/submodules/qtlocation-everywhere-src-6.9.1.tar.xz && \
    wget https://download.qt.io/official_releases/qt/6.9/6.9.1/submodules/qtpositioning-everywhere-src-6.9.1.tar.xz && \
    cd ../host-build && \
    tar xf ../src/qtserialport-everywhere-src-6.9.1.tar.xz && \
    tar xf ../src/qtserialbus-everywhere-src-6.9.1.tar.xz && \
    tar xf ../src/qtlocation-everywhere-src-6.9.1.tar.xz && \
    tar xf ../src/qtpositioning-everywhere-src-6.9.1.tar.xz && \
    echo "Compile qtserialport for host" && \
    cd qtserialport-everywhere-src-6.9.1 && \
    /build/qt6/host/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
    echo "Compile qtserialbus for host" && \
    cd ../qtserialbus-everywhere-src-6.9.1 && \
    /build/qt6/host/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
    echo "Compile qtpositioning for host" && \
    cd ../qtpositioning-everywhere-src-6.9.1 && \
    /build/qt6/host/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
    echo "Compile qtlocation for host" && \
    cd ../qtlocation-everywhere-src-6.9.1 && \
    /build/qt6/host/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
    cd ../../pi-build && \
    ...
    tar xf ../src/qtlocation-everywhere-src-6.9.1.tar.xz && \
    tar xf ../src/qtpositioning-everywhere-src-6.9.1.tar.xz && \
    echo "Compile qtserialport for rasp" && \
    cd qtserialport-everywhere-src-6.9.1 && \
    /build/qt6/pi/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
    echo "Compile qtserialbus for rasp" && \
    cd ../qtserialbus-everywhere-src-6.9.1 && \
    /build/qt6/pi/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
    echo "Compile qtpositioning for rasp" && \
    cd ../qtpositioning-everywhere-src-6.9.1 && \
    /build/qt6/pi/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
    echo "Compile qtlocation for rasp" && \
    cd ../qtlocation-everywhere-src-6.9.1 && \
    /build/qt6/pi/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
